...
public class GameState{
    ...
    public GameState(Context context, Sound soundSystem)
    {
    ...
    public boolean isBetween(float x, double lowerBound, double upperBound){
        if(x >= (360 - cornerVariation)){
            return true;
        }
        return (lowerBound - cornerVariation) <= x && x <= (upperBound + cornerVariation);
    }
